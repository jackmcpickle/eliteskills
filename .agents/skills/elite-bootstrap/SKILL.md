...
The output is a comprehensive plan document saved to `BOOTSTRAP-PLAN.md` in the project root, plus an optional `AGENTS.md` for ongoing Codex context.
...
- Generate a minimal `AGENTS.md` with **only CLI commands** (dev, build, test, lint, format, migrate, deploy). No descriptions, no architecture, no conventions — just commands.
...
6. Generate a minimal `AGENTS.md` containing **only** CLI commands:
