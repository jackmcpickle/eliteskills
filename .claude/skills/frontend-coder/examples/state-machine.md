...
export type Step =
  ...
  | { step: 3; data: Partial<IntegrationData>; account: AccountData; team: TeamData }
  | { step: 4; account: AccountData; team: TeamData; integrations: IntegrationData }
...
export type Action =
  ...
  | { type: 'UPDATE_INTEGRATION'; field: keyof IntegrationData; value: boolean }
  // Submission
  | { type: 'SUBMIT' }
  | { type: 'SUBMIT_SUCCESS' }
...
export function getFieldErrors(result: z.SafeParseReturnType<any, any>): Record<string, string> {
...
import { Model, Action, Step, WizardData, AccountData, TeamData, IntegrationData } from '../types'
import { validateStep1, validateStep2, validateStep3, getFieldErrors } from '../schemas'
    ...
    case 'BACK': {
      switch (current.step) {
        ...
        case 3:
          return {
            type: 'InProgress',
            current: { step: 2, data: current.team, account: current.account },
          }
        case 4:
          return {
            type: 'InProgress',
...
export function useSetupWizard(teamSlug: string) {
  ...
  const updateAccount = useCallback((field: keyof AccountData, value: string) => {
  ...
  const updateIntegration = useCallback((field: keyof IntegrationData, value: boolean) => {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/a/${teamSlug}/onboarding/api/setup/`, {
        method: 'POST',
      ...
      dispatch({ type: 'SUBMIT_ERROR', error: 'Failed to complete setup' })
    }
  }, [model, teamSlug])
  ...
  return {
    model,
    validation,
    actions: { next, back, updateAccount, updateTeam, updateIntegration, submit, retry },
  }
}
```
