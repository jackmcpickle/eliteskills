...
export type Model =
  ...
  | { type: 'SaveError'; data: Item[]; editingItem: Item; draft: ItemDraft; error: string }
...
export function isLoaded(model: Model): model is { type: 'Loaded'; data: Item[] } {
...
export function isEditing(model: Model): model is
  ...
  | { type: 'Saving'; data: Item[]; editingItem: Item; draft: ItemDraft }
  | { type: 'SaveError'; data: Item[]; editingItem: Item; draft: ItemDraft; error: string } {
  return model.type === 'Editing' || model.type === 'Saving' || model.type === 'SaveError'
}
