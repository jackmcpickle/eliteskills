...
const PRODUCTS: Record<string, ProductInfo> = {
	once: { name: 'Elite AI Skills - One-Time Purchase', price: 29, priceId: STRIPE_PRICE_ONCE ?? '' },
	lifetime: {
...
export function isStripeConfigured(): boolean {
	return Boolean(STRIPE_SECRET_KEY && STRIPE_PRICE_ONCE && STRIPE_PRICE_LIFETIME);
}
...
export async function getCheckoutSession(sessionId: string): Promise<StripeTypes.Checkout.Session> {
	const stripe = getStripeClient();
